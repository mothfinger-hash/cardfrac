// api/_lib/signup-confirmation-template.js
//
// HTML + plain-text template for the account-creation confirmation
// email. Used in two places:
//   • Standalone preview generator (preview_signup_confirmation.js) so
//     the design can be iterated without trial-emailing
//   • Pasted into Supabase Dashboard → Authentication → Email Templates
//     → "Confirm signup" so Supabase's auth flow sends this branded
//     email instead of its default text-only "Confirm your signup."
//
// Supabase substitutes these template variables when it sends:
//   {{ .ConfirmationURL }}  — one-time link that confirms the email +
//                              signs the user in for the first time
...
const PREVIEW_SUBSTITUTIONS = {
  '{{ .ConfirmationURL }}': 'https://pathbinder.gg/?type=signup&token=PREVIEW',
  '{{ .SiteURL }}':         'https://pathbinder.gg',
  '{{ .Email }}':           'you@example.com',
};

// `siteOrigin` controls where pb_logo.png + dash2.webp + noise.png
// resolve from. Defaults to pathbinder.gg; the preview script
// overrides this to use relative paths so the images load from
// local disk.
...
<head>
  ...
  <title>Confirm your PathBinder account</title>
</head>
        ...
        <table role="presentation" width="540" cellspacing="0" cellpadding="0" border="0" style="max-width:540px;width:100%;background:rgba(10,14,26,0.94);border:1px solid #1AC7A0;border-radius:14px;box-shadow:0 0 0 1px rgba(26,199,160,0.4), 0 0 28px rgba(26,199,160,0.35), 0 0 64px rgba(26,199,160,0.18)">
          ...
          <tr>
            <td style="padding:8px 36px 0">
              <h1 style="font-family:Helvetica,Arial,sans-serif;font-size:22px;font-weight:800;color:#ffffff;letter-spacing:0.04em;margin:18px 0 16px;text-align:center">
                Welcome to PathBinder.
              </h1>
              <p style="font-size:14px;color:#d8e0e8;line-height:1.65;margin:0 0 18px;text-align:center">
                You&rsquo;re one click away from a binder that tracks itself.
                Confirm your email and we&rsquo;ll drop you straight into the app.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:8px 36px 8px;text-align:center">
              ...
              <div style="font-size:11px;color:#6a7888;margin-top:14px">
                ...
                After that, sign in again and we&rsquo;ll resend a fresh one.
              </div>
            </td>
          </tr>

          <!-- "What's inside PathBinder" — quick orientation so new users
               know what they're walking into before they click. -->
          <tr>
                ...
                <tr>
                  <td style="padding:16px 20px">
                    ...
                    <ul style="margin:0;padding:0 0 0 18px">
                      <li style="margin:0 0 8px;color:#d8e0e8;font-size:14px;line-height:1.55">Scanner-fed binder &mdash; snap a card, it lands in your collection with live market value attached</li>
                      <li style="margin:0 0 8px;color:#d8e0e8;font-size:14px;line-height:1.55">Watchlist with price alerts on cards you want, before you own them</li>
                      <li style="margin:0 0 8px;color:#d8e0e8;font-size:14px;line-height:1.55">Marketplace where you can sell to other collectors at a flat low fee</li>
                      <li style="margin:0 0 8px;color:#d8e0e8;font-size:14px;line-height:1.55">Multi-TCG support &mdash; Pokemon, Magic, Yu-Gi-Oh, One Piece, and more</li>
                    </ul>
                  </td>
                </tr>
              </table>
          ...
          <!-- Didn't sign up? — the standard signup-attack disclaimer. -->
          <tr>
                ...
                <tr>
                  <td style="padding:14px 18px">
                    ...
                    <div style="font-size:13px;color:#d8e0e8;line-height:1.6">
                      ...
                      email &mdash; the unconfirmed account expires automatically and
                      no one can use it without confirming via the link above.
                    </div>
                  </td>
                </tr>
              </table>
...
function renderText({ forPreview }) {
  let text = [
    '◈ PATHBINDER — Confirm Your Account ◈',
    '',
    "Welcome to PathBinder. You're one click away from a binder that",
    'tracks itself. Confirm your email and we\'ll drop you straight into',
    'the app.',
    '',
    '{{ .ConfirmationURL }}',
    '',
    ...
    '',
    "What's inside:",
    '  - Scanner-fed binder — snap a card, it lands with live market value',
    '  - Watchlist + price alerts on cards you want',
    '  - Marketplace with flat low fees',
    '  - Multi-TCG: Pokemon, Magic, Yu-Gi-Oh, One Piece, and more',
    '',
    "Didn't sign up? Ignore this email — the unconfirmed account",
    'expires automatically and no one can use it.',
    '',
    '— PathBinder',
    '{{ .SiteURL }}',
  ].join('\n');
